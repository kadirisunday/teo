        ...
        <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
            {
                doctors.slice(0,10).map((items,index)=>(
                    <div onClick={()=>navigate(`./appointment/${items._id}`)} className='border border-red-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] trasition-all duration-500' key={index}>
